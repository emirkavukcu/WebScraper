...
def extract_content_from_page(file_path):
    ...
    datetime_content = str(time_element['datetime'])[:10]
    parsed_data["date"] = datetime_content

    #Extracting Title
    title_element = soup.find('div', {'class': 'page-header'}).find('h3').text.strip()
    parsed_data["title"] = title_element

    #Extracting Content
    content_element = soup.find('div', itemprop="articleBody").text.strip()
    parsed_data["content"] = content_element
    print(parsed_data)
